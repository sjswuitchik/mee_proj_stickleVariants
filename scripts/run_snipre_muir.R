...
# read in data 
snipre_data <- read_tsv("muir_snipre_data.tsv", col_names = T)
...
write.table(MKtest, "muir_mk_output.tsv", sep = "\t", quote = F, row.names = F)
...
write.table(snipre.qres, "muir_snipre_output.tsv", sep = "\t", quote = F, row.names = F)
