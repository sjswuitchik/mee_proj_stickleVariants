...
# read in data 
snipre_data <- read_tsv("shunda_snipre_data.tsv", col_names = T)
...
write.table(MKtest, "shunda_mk_output.tsv", sep = "\t", quote = F, row.names = F)
...
write.table(snipre.qres, "shunda_snipre_output.tsv", sep = "\t", quote = F, row.names = F)
